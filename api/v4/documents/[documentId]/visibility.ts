"use strict";
/* Source: api/v4/documents/[documentId]/visibility.ts */

// lib/supabase.ts (inlined for bundle)
...
async function supabaseRest(table, options = {}) {
  ...
  for (const [k, v] of Object.entries(filters)) reqUrl.searchParams.set(k, v);
  const headers = {
    apikey: key,
...
/**
 * PATCH /api/v4/documents/:documentId/visibility
 * Body: { isPublic: boolean }
 *
 * Updates the is_public flag on a document.
 * Ownership enforcement is delegated to RLS — the service-role key bypasses
 * RLS here, so we explicitly verify owner_id matches the x-auth-user-id header.
 * In production, replace the header check with a proper JWT verification.
 */
export default async function handler(req, res) {
  ...
  // documentId comes from the Vercel dynamic route segment.
  ...
  // Caller identity — expect Supabase user UUID in x-auth-user-id header.
  // In production this should be validated against your JWT.
  const callerId = Array.isArray(req.headers["x-auth-user-id"])
    ? req.headers["x-auth-user-id"][0]

...
export function PavilionSessionView(props: PavilionSessionViewProps) {
	...
	const [activeTab, setActiveTab] = useState<"analysis" | "blueprint" | "concept-map" | "fingerprint" | "student" | "plan" | "preview" | "class">("analysis");
	...
	useEffect(() => {
		if (activeTab === "blueprint" && !instructionalSession?.blueprint) {
			void loadBlueprint(sessionId);
		}
	}, [activeTab, instructionalSession?.blueprint, loadBlueprint, sessionId]);

	useEffect(() => {
		if (activeTab === "fingerprint" && teacherId && !instructionalSession?.teacherFingerprint) {
			void loadTeacherFingerprint(teacherId);
		}
	}, [activeTab, instructionalSession?.teacherFingerprint, loadTeacherFingerprint, teacherId]);

	useEffect(() => {
		if (activeTab === "plan" && !instructionalSession?.builderPlan) {
			void loadBuilderPlan(sessionId, instructionalSession?.activeStudentId);
		}
	}, [activeTab, instructionalSession?.activeStudentId, instructionalSession?.builderPlan, loadBuilderPlan, sessionId]);

	useEffect(() => {
		if (activeTab === "preview" && !instructionalSession?.assessmentPreview) {
			void loadAssessmentPreview(sessionId, instructionalSession?.activeStudentId);
		}
	}, [activeTab, instructionalSession?.activeStudentId, instructionalSession?.assessmentPreview, loadAssessmentPreview, sessionId]);
	...
	async function handleGenerateProduct() {
		...
		try {
			...
			if (product) {
				...
				setStatus(`${resolveProductTitle(product)} generated.`);
		...
		<div className="v4-product-grid">
			<div className="v4-product-card v4-product-span">
				<div className="v4-tab-strip" role="tablist" aria-label="Pavilion tabs">
					<button className={`v4-tab-button ${activeTab === "analysis" ? "v4-tab-button-active" : ""}`} type="button" role="tab" aria-selected={activeTab === "analysis"} onClick={() => setActiveTab("analysis")}>Analysis</button>
					...
					<button className={`v4-tab-button ${activeTab === "student" ? "v4-tab-button-active" : ""}`} type="button" role="tab" aria-selected={activeTab === "student"} onClick={() => setActiveTab("student")}>Student</button>
					...
					<div className="v4-product-card v4-product-span">
						<div className="v4-section-heading">
							<div>
								...
								<h2>Generate a classroom surface</h2>
							</div>
							<span className="v4-pill">{sortedProducts.length} outputs</span>
						</div>
							...
							{sortedProducts.length > 0 ? sortedProducts.map((product) => (
								<li key={product.productId}>
									<button className={`v4-history-item ${selectedProduct?.productId === product.productId ? "v4-history-item-active" : ""}`} type="button" onClick={() => setSelectedProductId(product.productId)}>
										<strong>{resolveProductTitle(product)}</strong>
										<span>{new Date(product.createdAt ?? product.payload.generatedAt ?? Date.now()).toLocaleDateString()}</span>
									</button>
								</li>
							)) : <li><p className="v4-body-copy">Generate a surface to populate the pavilion output gallery.</p></li>}
						</ul>
					</div>
					<div className="v4-product-card v4-product-span">
						<h3>Selected Output</h3>
						{selectedProduct ? (
							selectedProduct.intentType === "build-test"
								? <p className="v4-body-copy">Assessment outputs are rendered across the Blueprint, Concept Map, Student, Builder Plan, Living Assessment, and Class tabs.</p>
								: renderProduct(selectedProduct)
						) : (
							<p className="v4-body-copy">No pavilion output is selected yet.</p>
						)}
					</div>
				</>
			) : null}
			...
			{activeTab === "plan" ? <BuilderPlanView plan={instructionalSession?.builderPlan ?? null} onRefresh={() => { void loadBuilderPlan(sessionId, instructionalSession?.activeStudentId); }} /> : null}
